...
describe("insertPublicComment — new", () => {
  it("wraps the chosen occurrence in a guest comment thread", () => {
    ...
    expect(out).toContain(
      'The cat sat. The {==cat==}{>>which one?<<}{id="g1" by="Jane \\"JD\\"" at="2026-06-18T00:00:00.000Z" guest="true"} ran.',
    );
  });
  ...
  it("throws AnchorError when the occurrence does not exist", () => {
    expect(() =>
      insertPublicComment(base("hello\n"), {
        mode: "new",
        quote: "cat",
        occurrence: 1,
        text: "x",
  ...
  it("throws AnchorError when text contains <<}", () => {
    expect(() =>
      insertPublicComment(base("hello world\n"), {
        mode: "new",
        quote: "hello",
        occurrence: 1,
        text: "a <<} b",
  ...
  it("throws AnchorError when occurrence is 0", () => {
    expect(() =>
      insertPublicComment(base("The cat sat.\n"), {
        mode: "new",
        quote: "cat",
        occurrence: 0,
        text: "x",
...
describe("insertPublicComment — reply", () => {
  const thread = (body: string) =>
    `---\npublic: true\ncomments: true\n---\n${body}`;
  const doc = thread(
    'See {==here==}{>>first<<}{id="c1" by="A" at="t1"} now.\n',
  );
  ...
  it("appends a reply block carrying re=<parentId> right after the parent block", () => {
    ...
    expect(out).toContain(
      '{id="c1" by="A" at="t1"}{>>agreed<<}{id="c2" by="Bob" at="2026-06-18T00:00:00.000Z" re="c1" guest="true"}',
    );
  });

  it("throws AnchorError when the parent id is absent", () => {
    expect(() =>
      insertPublicComment(doc, {
        mode: "reply",
        parentId: "nope",
        text: "x",
        authorName: "B",
